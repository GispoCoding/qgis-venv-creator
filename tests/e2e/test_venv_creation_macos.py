...
# Mark the whole module to run only on MacOs
pytestmark = [pytest.mark.macos, pytest.mark.e2e]
...
class TestVenvCreation:
    ...
    def test_qgis_is_importable(self, venv_parent: Path):
        try:
            p = subprocess.run(
                'source .venv/bin/activate && python -c "from qgis.core import Qgis;print(Qgis.version())"',
                cwd=venv_parent,
                capture_output=True,
                shell=True,
                text=True,
